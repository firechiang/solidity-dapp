...
/**
 * 交易配对合约（就是流动性交易对合约）
 * 注意：该合约由UniswapV2Factory（工厂合约）动态创建部署。每一个流动性交易对都会部署一个这合约
 */
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function _safeTransfer(address token, address to, uint value) private {
         //通过token合约地址来调用合约的transfer方法，进行转账
        ...
        if (feeOn) {
            ...
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    // 如果流动性 > 0 将流动性铸造给feeTo地址（就是将手续费打给feeTo地址）
    ...
    /**
	 * 销毁方法（就是取出流动性，也是取走你往流动性池子里面添加的代币）
     * @param to to地址
     * @return amount0
     * @return amount1
     * @notice 应该从执行重要安全检查的合同中调用此低级功能
